...
foreach ($block in $blocks) {
    ...
    # TOTAL PSS
    $totalPss = $null
    if ($text -match 'TOTAL PSS:\s+(?<pss>\d+)') {
    ...
    if ($text -match '(?<user>\d+)%user.*?(?<sys>\d+)%sys.*?(?<idle>\d+)%idle.*?(?<irq>\d+)%irq.*?(?<sirq>\d+)%sirq') {
    ...
    # process cpu
    $procCpu = $null
    $procLine = $block | Where-Object { $_ -match 'io\.agora\.entfull' } | Select-Object -First 1
    if ($procLine -and $procLine -match 'S\s+(?<cpu>[\d\.]+)') {
        $procCpu = [double]$Matches.cpu
    }
